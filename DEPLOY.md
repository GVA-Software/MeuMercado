...
O app persiste o estado (usuários, assinaturas, carrinhos) em **Postgres** quando
`DATABASE_URL` está definido (sem ele, cai para memória — útil no dev local). Com isso,
roda bem tanto na **Vercel** (serverless + Postgres) quanto em hosts de processo como o
**Render** (onde a própria API serve a PWA, single-origin). Escolha um caminho abaixo.

---

## Opção Vercel (PWA + API serverless + Postgres)

Agora com persistência (Postgres), a Vercel funciona: ela serve a PWA e roteia
`/api/*` para uma **função serverless** que roda a API. O estado vive no Postgres.

1. **Crie um Postgres grátis** em https://neon.tech → copie a **"Pooled connection"**
   (formato `postgresql://...-pooler...neon.tech/neondb?sslmode=require`).
2. Em https://vercel.com/new → importe **GVA-Software/MeuMercado**.
3. A Vercel lê o [`vercel.json`](vercel.json). Em **Environment Variables**, defina:
   | Variável             | Valor                                                |
   | -------------------- | ---------------------------------------------------- |
   | `DATABASE_URL`       | a connection string **pooled** do Neon               |
   | `JWT_ACCESS_SECRET`  | segredo forte (≥32) — ex.: `openssl rand -base64 48` |
   | `JWT_REFRESH_SECRET` | outro segredo forte (≥32)                            |
   | `COOKIE_SECURE`      | `true`                                               |
   > `NODE_ENV=production` é automático na Vercel. As tabelas são criadas sozinhas (`DB_SYNCHRONIZE` default).
4. **Deploy** → você recebe a URL de **PROD**. Cada branch/PR ganha uma **URL de preview** (o "DEV") automaticamente.

> A senha usa **scrypt** (embutido no Node) — sem binário nativo, 100% compatível com serverless.
